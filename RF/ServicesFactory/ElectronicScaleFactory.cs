...
namespace WindowsFormsApplication1.ServicesFactory
{
    public class ElectronicScaleFactory : EquipmentFactory
    {
        ...
        {
            // 获取设备信息
            ElectronicScaleService service = new ElectronicScaleService();
    ...
    public class ElectronicScaleService : WebSocketBehavior
    {
        ...
        private void OpenCom()
        {
            if (isOpen == true)
            {
                ...
                {
                    ServiceLog.WriteServiceLog(ServiceLog.ES_SERVICE, "关闭串口时发生错误", "", DateTime.Now);
            ...
            if (isOpen == false)
            {
                ...
                {
                    ServiceLog.WriteServiceLog(ServiceLog.ES_SERVICE, "串口未设置", "", DateTime.Now);
                    return;
                }
                ...
                try
                {
                    ...
                    ServiceLog.WriteServiceLog(ServiceLog.ES_SERVICE, "启动串口", "", DateTime.Now);
                }
                catch (Exception)
                {
                    //打开串口失败后，相应标志位取消
                    isSetProperty = false;
                    isOpen = false;
                    ServiceLog.WriteServiceLog(ServiceLog.ES_SERVICE, "串口无效或已被占用", "", DateTime.Now);
                }
            }
        ...
        private void CloseCom()
        {
            try       //关闭串口       
            {
                sp.Close();
                isOpen = false;
                ServiceLog.WriteServiceLog(ServiceLog.ES_SERVICE, "关闭串口", "", DateTime.Now);
            }
            catch (Exception)
            {
                ServiceLog.WriteServiceLog(ServiceLog.ES_SERVICE, "关闭串口时发生错误", "", DateTime.Now);
